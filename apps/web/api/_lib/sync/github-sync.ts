import { getProductById, getAllProducts } from '../db/products.js';
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const node of data.items.nodes) {
        ...
        try {
          const existingTask = await getTaskByGitHubIssue(repoFullName, issue.number);
...
/** Sync all products, catching errors per product */
  ...
  for (const product of products) {
    try {
      ...
      results.set(product.id, {
        ...
        updated: 0,
        closed: 0,
        errors: [`Failed to sync product ${product.id}: ${error.message}`],
      });
    }
  }
